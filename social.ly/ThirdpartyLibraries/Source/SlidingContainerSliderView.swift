...
public class SlidingContainerSliderView: UIScrollView, UIScrollViewDelegate {
  ...
  public var sliderHeight: CGFloat = CGFloat(SwipeView.swipeViewHeight)
  ...
  public init(width: CGFloat, imagesNames: [String]) {
  ...
  public func draw() {
    ...
    if appearance.fixedWidth {
      var labelTag = 0
      let width = CGFloat(frame.size.width) / CGFloat(imagesNames.count)

      for title in imagesNames {
        
        let imageView = UIImageView.init(frame :CGRect.zero)
        imageView.tag = labelTag + 1
        let label = labelWithTitle(title,imageView: imageView)
        ...
        addSubview(label)
        let imageBackGroundView = UIView.init(frame: CGRect(x : label.frame.origin.x, y : 0, width : label.frame.size.width, height : 80 ))
        imageBackGroundView.backgroundColor = Color.intermidiateBackground.value
      //  var imageViewFrame =  CGRect(x : CGFloat(5) , y : CGFloat(0 + 10), width : imageBackGroundView.frame.size.width - CGFloat(10), height : CGFloat(80 - 20) )
        var imageViewFrame =  CGRect.zero
        imageViewFrame.size = (UIImage.init(named: title)?.size)!
      
        imageViewFrame.origin.x = (imageBackGroundView.frame.size.width - imageViewFrame.size.width)/CGFloat(2.0)
        imageViewFrame.origin.y = (imageBackGroundView.frame.size.height - imageViewFrame.size.height)/CGFloat(2.0)
       ...
       // imageView.center = imageBackGroundView.center
        ...
        imageView.image = UIImage.init(named: title)
    ...
    public func labelWithTitle(_ title: String, imageView : UIImageView = UIImageView()) -> UILabel {
