...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    var lgController : InitialVC!
    var sideMenuVC : SideMenuVC = SideMenuVC()
    
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        // Override point for customization after application launch.
        GoogleManager.sharedInstance.onLaunchOptions()
        self.initStoryBoardAndSideMenu()
    ...
    public func application(_ app: UIApplication, open url: URL, options: [UIApplicationOpenURLOptionsKey : Any] = [:]) -> Bool{
        ...
        let strAppID : String = FBSDKSettings.appID()
        let isFacebookURL = (url.scheme?.hasPrefix("fb\(strAppID)"))! && url.host == "authorize"
        if isFacebookURL {
            let strinngName = options[UIApplicationOpenURLOptionsKey.sourceApplication] as! String!
    ...
    //MARK: SideMenuVC
    func initStoryBoardAndSideMenu(){
        storyBoard = UIStoryboard(name: "Main", bundle: nil)
        lgController  = storyBoard.instantiateViewController(withIdentifier: "InitialVC") as! InitialVC
